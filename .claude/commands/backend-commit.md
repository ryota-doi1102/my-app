...
- Node.js + Hono + TypeScript（strict モード）
...
`backend/` ディレクトリで以下を順番に実行し、すべてパスすることを確認する。
...
| `db` | スキーマ変更・マイグレーションの追加 |

#### 例

```
feat: ユーザー認証エンドポイントを追加
fix: JWT の有効期限切れ時のエラーレスポンスを修正
db: users テーブルに role カラムを追加
test: ヘルスチェックエンドポイントのテストを追加
chore: drizzle-kit をアップデート
```
