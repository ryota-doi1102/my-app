...
describe("API-AUTH-05 POST /api/v1/auth/signout", () => {
	beforeEach(() => {
		vi.mocked(authService.signout).mockResolvedValue(undefined as never);
	});
	...
	describe("分岐処理", () => {
		...
		it("TC-402: 存在しないリフレッシュトークンを指定する", async () => {
			vi.mocked(authService.signout).mockRejectedValueOnce(
				new AppError(401, "UNAUTHORIZED", "トークンが無効です"),
			);
			const res = await post(VALID_BODY);
	...
	describe("エラー（500系）", () => {
		it("TC-501: DB接続失敗時に500を返す", async () => {
			vi.mocked(authService.signout).mockRejectedValueOnce(new Error("DB connection failed"));
			const res = await post(VALID_BODY);
		...
		it("TC-503: NODE_ENV=production時にスタックトレースがレスポンスに含まれない", async () => {
			...
			vi.mocked(authService.signout).mockRejectedValueOnce(new Error("DB error"));
			const res = await post(VALID_BODY);
			expect(res.status).toBe(500);
			const json = (await res.json()) as ErrorResponse;
