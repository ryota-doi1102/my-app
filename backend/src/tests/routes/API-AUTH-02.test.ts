...
function post(body: unknown, headers?: Record<string, string>) {
	return app.request("/api/v1/auth/signup", {
		method: "POST",
...
describe("API-AUTH-02 POST /api/v1/auth/signup", () => {
	beforeEach(() => {
		vi.mocked(authService.signup).mockResolvedValue({ accessToken: "mock-access-token" });
	});
	...
	describe("リクエストチェック（ボディ）", () => {
		...
		describe("email", () => {
			...
			it("TC-307: emailにメールアドレス形式でない文字列を指定する", async () => {
			...
			it("TC-308: emailに256文字の文字列を指定する", async () => {
		...
		describe("password", () => {
			...
			it("TC-310: passwordに7文字の文字列を指定する", async () => {
			...
			it("TC-311: passwordに256文字の文字列を指定する", async () => {
			...
			it("TC-312: passwordに大文字を含まない文字列を指定する", async () => {
			...
			it("TC-313: passwordに小文字を含まない文字列を指定する", async () => {
				const res = await post({ ...VALID_BODY, password: "PASSWORD1" });
				expect(res.status).toBe(422);
				const json = (await res.json()) as ErrorResponse;
				expect(json.error.code).toBe("VALIDATION_ERROR");
			...
			it("TC-314: passwordに数字を含まない文字列を指定する", async () => {
	...
	// -------------------------------------------------------------------------
	describe("分岐処理", () => {
		it("TC-401: 有効なトークン・メールアドレス・パスワードを指定してリクエストを送信する", async () => {
		...
		it("TC-405: トークン発行時のメールアドレスと異なるメールアドレスを指定する", async () => {
			vi.mocked(authService.signup).mockRejectedValueOnce(
				new AppError(401, "UNAUTHORIZED", "トークンが無効または期限切れです"),
			);
			const res = await post({ ...VALID_BODY, email: "other@example.com" });
			expect(res.status).toBe(401);
			const json = (await res.json()) as ErrorResponse;
			expect(json.error.code).toBe("UNAUTHORIZED");
			expect(json.error.messages[0]).toBe("トークンが無効または期限切れです");
		...
		it("TC-406: 既に登録済みのメールアドレスを指定する", async () => {
			vi.mocked(authService.signup).mockRejectedValueOnce(
				new AppError(409, "CONFLICT", "このメールアドレスは既に登録済みです"),
			);
			const res = await post(VALID_BODY);
			expect(res.status).toBe(409);
			const json = (await res.json()) as ErrorResponse;
			expect(json.error.code).toBe("CONFLICT");
			expect(json.error.messages[0]).toBe("このメールアドレスは既に登録済みです");
		});
	});
	...
	describe("エラー（500系）", () => {
		it("TC-501: DB接続失敗時に500を返す", async () => {
			vi.mocked(authService.signup).mockRejectedValueOnce(new Error("DB connection failed"));
			const res = await post(VALID_BODY);
		...
		it("TC-503: NODE_ENV=production時にスタックトレースがレスポンスに含まれない", async () => {
			...
			vi.mocked(authService.signup).mockRejectedValueOnce(new Error("DB error"));
			const res = await post(VALID_BODY);
			expect(res.status).toBe(500);
			const json = (await res.json()) as ErrorResponse;
